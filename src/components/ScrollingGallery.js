...
color: ${props => props.selected ? 'var(--primary)' : 'lightgray' };
width: 0.1em;
height: 0.1em;
margin: 1px;

&:hover {
  color: ${props => props.selected ? 'var(--primary)' : ''};
          ...
          <ModalGallery>
            ...
            <ModalImageNumber show={!moreInfo}>
              {images.map((image, key) => {
                return (<ModalSelectedImage onClick={() => setimageNum(key)} selected={key === imageNum} fontSize='inherit' key={key} as={LensIcon}/>)
              })}
            </ModalImageNumber>
